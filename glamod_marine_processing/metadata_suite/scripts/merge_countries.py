"""Metadata suite merging countries module."""
...
def read_args():
    ...
    parser.add_argument(
        ...
        required=True,
        default="jobs.json",
        help="JSON file containing configuration and list of jobs to run",
    )
    parser.add_argument(
        "-countries",
        dest="country_file",
        required=False,
        help="JSON file containing list of countries to process",
        default=None,
    )
    parser.add_argument(
        "-index",
        ...
        required=False,
        type=int,
        help="Index of country to process",
        default=None,
    )
    parser.add_argument(
        "-country",
        dest="country",
        required=False,
        help="2 character country code to process",
        default=None,
    )
...
def merge_records(tmpDf, master, duplicateChecks, numeric_columns, to_add):
    """Check each callsign and ship name to see if records can be merged with existing."""
    to_add = []
    for idx in tmpDf.index.values:
        ...
        if matches.shape[0] > 0:
            ...
            distance = max(
                float(matches["sx"][id_match[0]] == tmpDf.loc[idx, "sx"]),
            ...
            if distance > 0.8:
                ...
                common = list(
                    set(list(tmpDf))
                    .intersection(list(matches))
                    .intersection(duplicateChecks)
                )
                ...
                matching_elements = (
                    tmpDf.loc[idx, common] == matches.loc[id_match[0], common]
                )
                # possible actions
                #  - merge and fill
                #  - merge and correct
                ...
                if matching_elements.all():  # exact match, merge dates and files
                    ...
                    master.at[id_match[0], "source_file"] = (
                        master.loc[id_match[0], "source_file"]
                        + ";"
                        + tmpDf.loc[idx, "source_file"]
                    )
                    if (tmpDf.loc[idx, "name"] != master.loc[id_match[0], "name"]) & (
                        tmpDf.loc[idx, "name"]
                        not in master.loc[id_match[0], "alt_names"]
                    ):
                        master.at[id_match[0], "alt_names"] = (
                            master.loc[id_match[0], "alt_names"]
                            + ";"
                            + tmpDf.loc[idx, "name"]
                        )
                else:
                    ...
                    missing_right = (
                        ...
                        | (matches.loc[id_match[0], common] == fmiss)
                    )
                    ...
                    if missing.all():
                        ...
                        if (
                            tmpDf.loc[idx, "name"] != master.loc[id_match[0], "name"]
                        ) & (
                            tmpDf.loc[idx, "name"]
                            not in master.loc[id_match[0], "alt_names"]
                        ):
                        ...
                        if len(right_columns) > 0:
                            master.at[id_match[0], right_columns] = tmpDf.loc[
                                idx, right_columns
                            ]
                    else:
...
def replace_nans(master):
    """Replace nans in master."""
    for column in master:
        ...
        if master[column].dtype == "float64":
            ...
            print(f"Unknown column type: {master[column].dtype}")
    return master
...
def write_country_file(
    ...
    """Write country file on disk."""
    master = pd.DataFrame()
    for job in jobs:
        schema = pub47schema(configpath + "./schemas/", job["schema"])
        input_file = (
            outputpath + "./split/" + os.path.basename(job["data_file"]) + "." + country
        )
        ...
        datain = datain.drop_duplicates(
            keep="first"
        )  # some duplicates are appearing from somewhere !
        ...
        invMap = {v: k for k, v in mapUse.items()}
        ...
        if master.shape[0] > 0:
            ...
            to_add = merge_records(tmpDf, master, duplicateChecks, numeric_columns)
        else:
        ...
        master = replace_nans(master)

    # final step is sort and addition of record numbers

    # assign UIDs to all records
    uid = master.apply(
        lambda x: "{}-{}-{}".format(x["callsign"], x["sx"], x["recruiting_country"]),
        axis=1,
    )
    ...
    for uid in uids:
        ...
        records.loc[records.index[0], "valid_from"] = records.loc[
            records.index[0], "valid_from"
        ] - relativedelta(months=12)
        records.loc[records.index[nrecs - 1], "valid_to"] = records.loc[
            records.index[nrecs - 1], "valid_to"
    ...
    master.to_csv(
        outputpath + "./master/master." + country + ".csv",
        index=False,
        sep="|",
        na_rep="NULL",
    )
...
def main(argv):
    ...
    for country in countries:
        ...
        write_country_file(
            control["jobs"],
            duplicateChecks=config["duplicateChecks"],
            country=country,
            mapping=mapping,
