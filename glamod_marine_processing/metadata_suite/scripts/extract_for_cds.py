"""Metadata suite extracting for CDS module."""
...
def bad_number(number):
    """Check whether number is invalid."""
    try:
        int(number)
        return False
    except Exception:
...
def read_args(argv):
    ...
    parser.add_argument(
        ...
        required=False,
        default="config.json",
        help="JSON file containing configuration settings",
    )
    parser.add_argument(
        "-schema",
        dest="schema",
        required=True,
        help="JSON file containing schema for master file",
        default=None,
    )
    parser.add_argument(
        "-index",
...
def main(argv):
    ...
    metadata = pd.read_csv(
        outputpath + "./master/master_all.csv",
        sep="|",
        low_memory=False,
        dtype="object",
    )
    ...
    metadata["meteorological_vessel_type"] = metadata[
        "meteorological_vessel_type"
    ].fillna(-1)
    metadata["meteorological_vessel_type"] = metadata[
        "meteorological_vessel_type"
    ].astype(int)
    ...
    records_with_thermometer = (
        (~metadata["thermometer1_height"].isna())
    ...
    records_with_barometer = (
        (~metadata["barometer1_height"].isna())
    ...
    records_with_anemometer = (
        (~metadata["anemometer1_height_loadline"].isna())
    ...
    metadata.at[records_with_thermometer, "thermometer1_id"] = metadata.loc[
        records_with_thermometer, :
    ].apply(lambda x: "{}-{}-TH1".format(x["uid"], x["record_number"]), axis=1)
    metadata.at[records_with_hygrometer, "hygrometer1_id"] = metadata.loc[
        records_with_hygrometer, :
    ].apply(lambda x: "{}-{}-HY1".format(x["uid"], x["record_number"]), axis=1)
    metadata.at[records_with_sea_thermometer, "sea_thermometer1_id"] = metadata.loc[
        records_with_sea_thermometer, :
    ].apply(lambda x: "{}-{}-ST1".format(x["uid"], x["record_number"]), axis=1)
    metadata.at[records_with_barometer, "barometer1_id"] = metadata.loc[
        records_with_barometer, :
    ].apply(lambda x: "{}-{}-BM1".format(x["uid"], x["record_number"]), axis=1)
    metadata.at[records_with_anemometer, "anemometer1_id"] = metadata.loc[
        records_with_anemometer, :
    ].apply(lambda x: "{}-{}-AN1".format(x["uid"], x["record_number"]), axis=1)
    ...
    print(f"\n{d}")
    row = metadata.loc[
        (d >= metadata["valid_from"]) & (d < metadata["valid_to"]), :
    ].copy()
    # if row.shape[0] == 0:
    #    continue
    ...
    dcount = 0
    if dups:
        print(f"{len( dup_ids )} / {row.shape[0]}")
        for dup in dup_ids:
            dcount += 1
            if int(10.0 * dcount / len(dup_ids)) % 10 == 0:
                print(".", end="")
            highest_vos_class = min(
                row.loc[row["callsign"] == dup, "meteorological_vessel_type"]
            )
            best_duplicate = row.loc[
                (row["callsign"] == dup)
                & (row["meteorological_vessel_type"] == highest_vos_class),
                :,
            ]
            ...
            if best_duplicate.shape[0] > 1:
                ...
                best_duplicate = best_duplicate.loc[
                    best_duplicate["valid_from"] == latest_valid_from
                ]
            if best_duplicate.shape[0] == 1:
                # add best duplicate to unique rows and drop from rejects
                unique_rows = pd.concat(
                    [unique_rows, best_duplicate], ignore_index=True, sort=False
                )
    ...
    rejects = pd.concat(
        [rejects, unique_rows.loc[msng_callsign.index,]], ignore_index=True, sort=False
    )
    ...
    field_map = {
        ...
        "name": "name",
        "observing_frequency": "freq",
        "meteorological_vessel_type": "vsslM",
        "vessel_type": "vssl",
        "automation": "automation",
        "recruiting_country": "rcnty",
        "valid_from": "valid_from",
        "valid_to": "valid_to",
        "uid": "uid",
        "thermometer1_height": "thmH1",
        "platH": "platH",
        "barometer1_height": "brmH1",
        "anmH": "anmH",
        "anemometer1_height_loadline": "anHL1",
        "windwave_observing_height": "wwH",
        "sst1_depth": "sstD1",
        "thermometer1_id": "th1",
        "hygrometer1_id": "hy1",
        "sea_thermometer1_id": "st1",
        "barometer_id": "bm1",
        "anemometer_id": "an1",
    }
    ...
    to_write.to_csv(
        outputpath + "./monthly/{}.csv".format(d.strftime("%Y-%m-%d")),
        index=False,
        sep="|",
    )
