"""Metadata suite combining master files module."""
...
def main(argv):
    """Metadata suite combining master files main function."""
    parser = argparse.ArgumentParser(
        description="Merge individual country metadata files to one master file"
    )
    parser.add_argument(
        "-config",
        dest="config",
    ...
    for country in countries:
        ...
        metadata = pd.read_csv(
            outputpath + f"./master/master.{country}.csv",
            sep="|",
            low_memory=False,
            dtype="object",
        )
