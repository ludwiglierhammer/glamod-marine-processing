...
def main(argv):
    """Main configuration source function."""
    ...
    source_template = pd.read_csv(
        template_file, sep="|", dtype="object", low_memory=False
    )
    ...
    imma_obj = imma(
        ...
        sections=["core", "ATTM1", "ATTM98"],
    )
    imma_obj.loadImma(
        filename, sections=["core", " 1", "98"], verbose=True, block_size=None
    )

    print(f" Data read, md5: {imma_obj.cksum}")

    print(imma_obj.data)

    datain = imma_obj.data[["core.lon", "core.lat", "attm1.dck", "attm1.sid"]].copy()
    print(datain)
    datain["source_id"] = datain.apply(
        lambda x: "{:03d}-{:03d}".format(int(x["attm1.sid"]), int(x["attm1.dck"])),
        axis=1,
    )
    ...
    summary = pd.merge(
        summary, source_template, how="left", left_index=True, right_index=True
    )

    # reset index
    summary.reset_index(inplace=True)
    summary["source_id"] = summary["source_id"].apply(
        lambda x: f"ICOADS-3-0-0T-{x}-{iyear:04d}-{imonth:02d}"
    )
    ...
    summary = summary.assign(source_file=f"IMMA1_R3.0.0T_{iyear:04d}-{imonth:02d}")
    ...
    outfile = f"{iyear:04d}-{imonth:02d}.csv"
