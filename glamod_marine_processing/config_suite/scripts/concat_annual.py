"""Configuration annual concatenation module."""
...
def main(argv):
    """Main annual concatenation function."""
    parser = argparse.ArgumentParser(
        description="Concatenate monthly summaries to annual"
    )
    parser.add_argument(
        "-work",
        dest="work",
    ...
    files = Path(work_directory).glob(f"**/stations-{iyear:04d}-*.csv")
    datain = None
    for file in files:
        print(str(file))
    ...
    outfile = work_directory + f"./stations_annual-{iyear:04d}.csv"
