...
def main(argv):
    ...
    parser = argparse.ArgumentParser(description='Concatenate annuals summaries to single file')
    parser.add_argument("-work", dest="work", required=True, default=None, \
                        help="Directory containing annual summaries")
    parser.add_argument("-index", dest="idx", required=True, default=None, type=int,\
    ...
    if idx == 0:
        ...
        files = sorted( Path(work_directory).glob("**/stations_annual-{}*.csv".format( decades[idx - 1] ) ) )
...
#                                    platform_type            = pd.NamedAgg(column='platform_type',             aggfunc='unique'),
    ...
    for field in unique_check:
        ...
        summary[field] = summary[ field ].apply(lambda x: list( x[ x != 'NULL' ] ) if ( isinstance(x, list) and len(x) > 1 ) else x  )
