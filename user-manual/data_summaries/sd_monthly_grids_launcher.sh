...
do
   ...
   echo $sid_dck"[1-$arrl]"
   echo $sid_dck_log_dir/"%I-"$run_id".o"
   echo $sid_dck_log_dir/\$LSB_JOBINDEX"-"$run_id".input"
   bsub -J $sid_dck"[1-$arrl]" -oo $sid_dck_log_dir/"%I-"$run_id".o" -eo $sid_dck_log_dir/"%I-"$run_id".o" -q short-serial -W $job_time_hhmm -M $job_memo_mbi -R "rusage[mem=$job_memo_mbi]" python $pyscript $sid_dck_log_dir/\$LSB_JOBINDEX"-"$run_id".input"
done
