...
for dt in rrule.rrule(rrule.MONTHLY, dtstart=start, until=stop):
    ...
    logging.info('Aggregating sources:')
    for sid_dck in sid_dck_list:
        logging.info('{}'.format(sid_dck))
    ...
    nreports_arr = canvas.points(cdm_table,'longitude','latitude',ds.count('observation_value')).assign_coords(time=dt).rename('counts')
