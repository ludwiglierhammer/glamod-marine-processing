...
def get_nreports_df():
    ...
    nreports_partition = pd.DataFrame(index = item_list,columns=['nreports'])
    ...
    for item in item_list:
        ...
        nreports_partition.loc[item] = item_df.sum().sum()
        
    nreports_partition['hundreds'] = [i[0] for i in  nreports_partition.index]
    nreports_partition['units'] = [i[1:] for i in  nreports_partition.index] 
    tuples_level2 =  [ (x[0],x[1]) for x in zip( nreports_partition['hundreds'], nreports_partition['units']) ]  
    nreports_partition.index = tuples_level2 #pd.MultiIndex.from_tuples(tuples_level2,names=['hundreds','units'])
    nreports_partition.drop(['hundreds','units'],axis=1,inplace=True)  
    
    return nreports_partition

def nreports_heatmap():
    ...
    fig_path = os.path.join(level2_path,'number_of_reports_per_{}'.format(partition))
    if partition == 'source':
        ...
        index_add = ''
        partition_ID = 'Ddd'
    heatmap = pd.DataFrame(index = tuples,columns=['nreports'])
    
    heatmap['nreports'] = nreports_df['nreports']
    ...
    heatmap2 = heatmap.reset_index().pivot(columns=column_name,index=index_name,values='nreports')
