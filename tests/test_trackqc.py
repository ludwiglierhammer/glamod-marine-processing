...
from glamod_marine_processing.qc_suite.modules.next_level_track_check_qc import calculate_speed_course_distance_time_difference
...
def test_no_trim():
    ...
    assert trim == 4.0
    assert np.all(arr == np.array([10.0, 4.0, 3.0, 2.0, 1.0]))  # this checks the array is not modifed by the function
...
def test_sd_no_trim():
    ...
    assert trim == 2.0
    assert np.all(arr == np.array([6.0, 1.0, 1.0, 1.0, 1.0]))  # this checks the array is not modifed by the function
...
def test_data(selection):
    if selection == 1:
        # stationary drifter
        vals = [{'ID': 'AAAAAAAAA', 'YR': 2003, 'MO': 12, 'DY': 1, 'HR': 12, 'LAT': 0.0, 'LON': 0.0, 'SST': 5.0},
        ...
        vals = [{'ID': 'AAAAAAAAA', 'YR': 2003, 'MO': 12, 'DY': 1, 'HR': 1, 'LAT': 0.0, 'LON': 0.0, 'SST': 5.0},
        ...
        vals = [{'ID': 'AAAAAAAAA', 'YR': 2003, 'MO': 12, 'DY': 1, 'HR': 12, 'LAT': 85.0, 'LON': 0.0, 'SST': 5.0},
