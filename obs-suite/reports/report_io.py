...
NOT ROBUST ENOUGH TO HANDLE DATA NOT PROGRESSING TO HIGHER LEVELS, IE. TO SHOW
AN IO THAT DOES NOT LEAD TO LEVELX DATA.... !!!!
...
def main():   
    ...
    paths['reports'] = os.path.join(data_path,release,source,level,'reports',sid_dck)
...
#    df_dict['io_history'].loc[:,'invalid'] = df_dict['io_history'][['md-invalid','dt-invalid','id-invalid']].sum(axis=1)
    ...
    #3.- No.reports plot
    if level_data:
        out_file = os.path.join(paths.get('reports'), '-'.join(['no_reports',release,update,'all-ts.png']))
        title = sid_dck + ' C3S data (all qualities)'
        plot_no_reports(df_dict['report_no_all'],index[0], index[-1],title,out_file)
        out_file = os.path.join(paths.get('reports'), '-'.join(['no_reports',release,update,'all-ts.psv']))
        df_dict['report_no_all'].dropna(how='all',axis=1).to_csv(out_file,sep='|',index_label='yr-mo')

        out_file = os.path.join(paths.get('reports'), '-'.join(['no_reports',release,update,'qcr0-qc0-ts.png']))
        title = sid_dck + ' C3S data (recommended quality)'
        plot_no_reports(df_dict['report_no_best'],index[0], index[-1],title,out_file)
        out_file = os.path.join(paths.get('reports'), '-'.join(['no_reports',release,update,'qcr0-qc0-ts.psv']))
