...
# to run the level1c conversion
#
# Within the sid-deck array, a subjob is only submitted if the corresponding
# source level (level1b) monthly file is available.
#
# If Inarg clean_previous is set to y, removes data files at the source level (level1b) of
...
do
  ...
  rm -rf $sid_deck_scratch_dir;mkdir -p $sid_deck_scratch_dir
   ...
   then
     ...
     jobid_c=$(nk_jobid bsub -J $sid_deck'L1Bc' -w "done($jobid)" -oo $sid_deck_scratch_dir/"%I.o_clean" -eo $sid_deck_scratch_dir/"%I.e_clean" -q short-serial -W 00:15 -M 100 -R "rusage[mem=100]" $scripts_directory/clean_level_data.sh $sid_deck $release $update $source $source_level)
   else
