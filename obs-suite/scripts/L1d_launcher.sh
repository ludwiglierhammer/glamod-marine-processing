...
# to run the level1d conversion
#
# Within the sid-deck array, a subjob is only submitted if the corresponding
# source level (level1c) monthly file is available.
#
# If Inarg clean_previous is set to y, removes data files at the source level (level1c) of
...
# <scratch_dir>/level1d/sid_deck
#
# Main paths are set sourcing r092009_setenv0.sh
#
# sid_deck_list_file is: sid-dck yyyy-mm yyyy-mm
#
# Usage: ./L1d_launcher.sh release update source_dataset list_file clean_previous(y|n)
...
do
  ...
  rm -rf $sid_deck_scratch_dir;mkdir -p $sid_deck_scratch_dir
	...
	do
	 ...
	 jobid=$(nk_jobid bsub -J $sid_deck'L1Dm'"[1-$counter]" -oo $sid_deck_scratch_dir/"%I.o_main" -eo $sid_deck_scratch_dir/"%I.e_main" -q short-serial -W $job_time -M $job_memo -R "rusage[mem=$job_memo]" $scripts_directory/L1d_main_job.sh $sid_deck $release $update $source)
	 bsub -J L1Dm_ok"[1-$counter]" -w "done($jobid[*])" -oo $sid_deck_scratch_dir/"b%I.oo" -eo $sid_deck_scratch_dir/"b%I.oe" -q short-serial -W 00:01 -M 10 -R "rusage[mem=10]" $scripts_directory/bsub_array_output_hdlr.sh 0 $sid_deck_scratch_dir 'main' 0 $level
   bsub -J L1Dm_er"[1-$counter]" -w "exit($jobid[*])" -oo $sid_deck_scratch_dir/"b%I.eo" -eo $sid_deck_scratch_dir/"b%I.ee" -q short-serial -W 00:01 -M 10 -R "rusage[mem=10]" $scripts_directory/bsub_array_output_hdlr.sh 1 $sid_deck_scratch_dir 'main' 0 $level
   if [ $clean_previous_level == 'y' ]
   then
     echo 'Previous level data ('$source_level') cleaning requested'
     jobid_c=$(nk_jobid bsub -J $sid_deck'L1Dc' -w "done($jobid)" -oo $sid_deck_scratch_dir/"%I.o_clean" -eo $sid_deck_scratch_dir/"%I.e_clean" -q short-serial -W 00:15 -M 100 -R "rusage[mem=100]" $scripts_directory/clean_level_data.sh $sid_deck $release $update $source $source_level)
   else
