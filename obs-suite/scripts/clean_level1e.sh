...
# Find file specific parameters in job_index.input file located in the level1e
...
to_remove_file=$code_directory/configuration_files/r092019_000000_level1e_remove.txt
ddata=$data_directory/r092019/ICOADS_R3.0.0T/level1e
dlog=$data_directory/r092019/ICOADS_R3.0.0T/level1e/log
dql=$data_directory/r092019/ICOADS_R3.0.0T/level1e/quicklooks
...
do
  ...
  ls $ddata/$sid_deck/*.psv | awk -v yeari==$iniyr -v yeare==$endyr 'substr($1,length($0)-25,4)<yeari {print substr($1,length($0)-25,4)}'
	ls $ddata/$sid_deck/*.psv | awk -v yeari==$iniyr -v yeare==$endyr 'substr($1,length($0)-25,4)>yeare {print substr($1,length($0)-25,4)}'
