...
do
  ...
  bsub -J $sid_deck'L2REP' -w "done($jobid_io)" -oo $sid_deck_scratch_dir/"L2_rep.e" -eo $sid_deck_scratch_dir/"L2_rep.e" -q short-serial -W $job_time_rep -M $job_memo_rep -R "rusage[mem=$job_memo_rep]" python $code_directory/reports/pdf_report_jinja_$level.py $data_directory $release $update $source $sid_deck $y_init $y_end $level2list
