  ...
  # just did the trick...
    ...
    if [ 0 -lt $(ls $source_filename 2>/dev/null | wc -w) ]
      ...
      then
        ...
        cp $process_config_file $sid_dck_log_dir/$counter.input
        python config_array.py $sid_dck_log_dir/$counter.input $data_directory $sid_dck $yyyy $mm
        ((counter++))
      fi
		else
	 ...
	 ((counter--))
   jobid=$(nk_jobid bsub -J $sid_dck$data_level"[1-$counter]" -oo $sid_dck_log_dir/"%I.o" -eo $sid_dck_log_dir/"%I.o" -q short-serial -W $job_time_hhmm -M $job_memo_mbi -R "rusage[mem=$job_memo_mbi]" \
   python $scripts_directory/$script_name $sid_dck_log_dir/\$LSB_JOBINDEX.input)
   ...
   python $scripts_directory/process_array_output_hdlr.py $sid_dck_log_dir/\$LSB_JOBINDEX.input 0 1
   ...
   python $scripts_directory/process_array_output_hdlr.py $sid_dck_log_dir/\$LSB_JOBINDEX.input 1 1
