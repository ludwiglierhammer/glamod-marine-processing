...
do
  ...
  rm -rf $sid_deck_scratch_dir;mkdir -p $sid_deck_scratch_dir
	...
	do
	 ...
	 jobid=$(nk_jobid bsub -J $sid_deck'L1Em'"[1-$counter]" -oo $sid_deck_scratch_dir/"%I.o_map" -eo $sid_deck_scratch_dir/"%I.e_map" -q short-serial -W $job_time -M $job_memo -R "rusage[mem=$job_memo]" $scripts_directory/L1e_maps_repeat_job.sh $sid_deck $release $update $source)
