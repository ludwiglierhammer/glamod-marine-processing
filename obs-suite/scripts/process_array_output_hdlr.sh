...
#
# Usage: ./process_array_output_hdlr.sh exit_status sid_deck_scratch_dir $process rm_input_files
...
mv $sid_deck_scratch_dir/$job_idx.o $job_log_dir/$file_id$ffs$process.$ext

# Consider removing *.input files when this is working....
if (( rm_input == 0 ));then rm $sid_deck_scratch_dir/$job_idx.input;fi
