...
Outputs data to /<data_path>/<release>/<source>/level1b/<sid-dck>/table[i]-fileID.psv
Outputs quicklook info to:  /<data_path>/<release>/<source>/level1b/quicklooks/<sid-dck>/fileID.json
where fileID is yyyy-mm-release_tag-update_tag

If any data in source yyyy-mm is identified to be in a different yyyy-mm (mainly after datetime corrections):
Outputs data to /<data_path>/<release>/<source>/level1b/<sid-dck>/table[i]-fileLeakID.psv        
where fileLeakID is yyyy-mm(real)-release_tag-update_tag-yyyy-mm(source)
...
config_file: path to configuration file with processing options
...
if not os.path.isfile(params.config_file):
    logging.error('Configuration file {} not found'.format(params.config_file))
    sys.exit(1)  
else:
