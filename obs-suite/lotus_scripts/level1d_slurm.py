...
for sid_dck in process_list:
    ...
    logging.info('{}: launching'.format(sid_dck)) 
    ...
    if remove_source:
        job_remove_file = os.path.join(log_diri,sid_dck + '_remove.slurm')
        taskfarm_remove_file = os.path.join(log_diri, sid_dck + '_remove.tasks')
