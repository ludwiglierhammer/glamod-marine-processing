...
VARLIST = ['YR', 'MO', 'DY', 'HR', 'LAT', 'LON', 'DS', 'VS', 'SLP', 'AT',
            'AT2', 'SST', 'DCK', 'PT', 'SID', 'DPT', 'SHU', 'VAP', 'CRH', 
            'CWB', 'DPD', 'W', 'WI','D', 'DI', 'WW']
...
class Voyage:
    ...
    def track_check(self, parameters):
        ...
        for i in range(1, nobs - 1):
            ...
            if (self.getvar(i, 'speed') is not None and self.getvar(i, 'speed') > amax and
                    self.getvar(i - 1, 'alt_speed') is not None and self.getvar(i - 1, 'alt_speed') > amax):
                thisqc_a += 1.00
            elif (self.getvar(i + 1, 'speed') is not None and self.getvar(i + 1, 'speed') > amax and
                  self.getvar(i + 1, 'alt_speed') is not None and self.getvar(i + 1, 'alt_speed') > amax):
                thisqc_a += 2.00
            elif (self.getvar(i, 'speed') is not None and self.getvar(i, 'speed') > amax and
                  self.getvar(i + 1, 'speed') is not None and self.getvar(i + 1, 'speed') > amax):
                thisqc_a += 3.00
